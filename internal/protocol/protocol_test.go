...
func TestChatGPTProtocolDescribesSubmissionAndControlSchemas(t *testing.T) {
	...
	for _, want := range []string{
		...
		"evidence_anchor", "kind 只支持 onscreen / offscreen", "events.json 不得自行提供 canon_id", "event_ref 与 event_canon_id 不得同时提供", "event_ref 与 event_canon_id 字段键不得同时出现", "event_canon_id 一旦出现必须是非空字符串", "actors/observers 一旦出现必须是字符串数组", "actors/observers 数组内不得重复人物引用", "kind=offscreen", "离屏事件", "hard_constraints[].id", "state_delta.json", "state_delta.json.changes 必须是数组", "event_ref", "event_ref 一旦出现必须是非空字符串", "self_review.json", "self_review.ok 必须是 boolean", "ok:false 必须同时提供 author_decision_required", "ok:true 不得同时提供 author_decision_required",
