...
func TestChatGPTProtocolDescribesSubmissionAndControlSchemas(t *testing.T) {
	...
	for _, want := range []string{
		...
		"evidence_anchor", "kind 只支持 onscreen / offscreen", "kind=offscreen", "离屏事件", "hard_constraints[].id", "state_delta.json", "state_delta.json.changes 必须是数组", "event_ref", "self_review.json",
