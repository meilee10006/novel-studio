...
func TestChatGPTProtocolDescribesSubmissionAndControlSchemas(t *testing.T) {
	...
	for _, want := range []string{
		...
		"foundation.json", "title", "characters.json", "characters 必须是数组", "world.json", "world.entities 一旦出现必须是数组", "local_id", "entity_type", "local_ref 一旦出现必须是非空字符串", "Foundation 首次定义不得自行提供 canon_id",
		...
		"evidence_anchor", "kind 只支持 onscreen / offscreen", "events.json 不得自行提供 canon_id", "event_ref 与 event_canon_id 不得同时提供", "actors/observers 一旦出现必须是字符串数组", "actors/observers 数组内不得重复人物引用", "kind=offscreen", "离屏事件", "hard_constraints[].id", "state_delta.json", "state_delta.json.changes 必须是数组", "event_ref", "event_ref 一旦出现必须是非空字符串", "self_review.json", "self_review.ok 必须是 boolean", "ok:false 必须同时提供 author_decision_required", "ok:true 不得同时提供 author_decision_required",
		...
		"character_add", "location_add", "resource_add", "description 一旦出现必须是字符串", "同一提交", "id_mappings", "首次创建不得自行提供永久 ID 字段",
		...
		"relationship_id", "两个不同 canonical characters", "tags", "tags 一旦出现必须是唯一非空字符串数组", "change kind 必须来自支持列表", "foreshadow_id", "foreshadow.description", "首次创建伏笔使用 local_id", "永久 foreshadow ID", "description 创建后不可改写", "payoff_ready", "paid_off", "closed",
		"promise_id", "reader_promise.statement", "首次创建读者承诺使用 local_id", "永久 reader promise ID", "statement 创建后不可改写", "advanced", "fulfilled", "deferred", "deadline_chapter", "retired", "fulfilled / retired 为终态",
