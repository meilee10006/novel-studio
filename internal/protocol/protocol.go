...
knowledge_add 的 fact_id 是稳定标识，statement 是后续新对话可直接理解的事实文本；新增知识应同时提供两者。observed source 要求对应 events.json 事件把该角色列在 observers 中，例如 {"local_id":"e1","evidence_anchor":"...","observers":["character-000001"]}。transmitted source 必须提供 from_character_id；from_character_id 必须出现在 event actors，接收角色必须出现在 event observers，并且来源角色在前态已经知道同一 fact_id。旧 Canon 中没有 statement 的知识仍按 fact_id 兼容读取。relationship 的 tags 可省略；tags 一旦出现必须是唯一非空字符串数组。resource、location、relationship、foreshadow、conflict、ending_resolution 需要事件证据。
