...
knowledge_add 的 fact_id 是稳定标识，statement 是后续新对话可直接理解的事实文本；新增知识应同时提供两者。同一角色已有非空 statement 时不得改写；后续省略 statement 时 Core 保留既有文本。observed source 要求对应 events.json 事件把该角色列在 observers 中，例如 {"local_id":"e1","evidence_anchor":"...","observers":["character-000001"]}。transmitted source 必须提供 from_character_id；from_character_id 必须出现在 event actors，接收角色必须出现在 event observers，并且来源角色在前态已经知道同一 fact_id。transmitted 不得改写来源 fact 的 statement；来源已有 statement 时可省略该字段，由 Core 继承来源文本。旧 Canon 中没有 statement 的知识仍按 fact_id 兼容读取。relationship 的 tags 可省略；tags 一旦出现必须是唯一非空字符串数组。resource、location、relationship、foreshadow、conflict、ending_resolution 需要事件证据。location.start_tick 不得早于该角色前态 end_tick；地点变化时若存在匹配的 travel_constraints，还必须满足对应 min_ticks。

首次创建伏笔使用 local_id，并提供 foreshadow.description 作为后续新对话可读的伏笔语义；只有 ACCEPTED 时 Core 才分配永久 foreshadow ID，并通过 id_mappings 返回，后续状态推进使用 foreshadow_id 字段填写该 canonical ID。description 创建后不可改写；后续推进若省略 description，Core 保留前态描述，显式提供时必须与已有非空 description 一致。旧 Canon 已存在的历史伏笔 key 仍兼容继续推进。foreshadow 合法推进主路径为 planned → seeded → reinforced → payoff_ready → paid_off → closed，也可在允许阶段 retired；final export 前所有 foreshadow 必须是 closed 或 retired。
...
首次创建读者承诺使用 local_id，并提供 reader_promise.statement；只有 ACCEPTED 时 Core 才分配永久 reader promise ID，并通过 id_mappings 返回，后续状态推进使用 promise_id 字段填写该 canonical ID。statement 创建后不可改写；后续推进若省略 statement，Core 保留前态文本，显式提供时必须与已有非空 statement 一致。旧 Canon 已存在的历史 promise key 仍兼容继续推进。reader_promise 可用 advanced、deferred、fulfilled、retired；deferred 必须带未来的 deadline_chapter，fulfilled 必须有 event_ref；fulfilled / retired 为终态，不得恢复为 advanced/deferred；final export 前必须是 fulfilled 或 retired。最终主线收束还必须提交带本章事件证据的 ending_resolution。
