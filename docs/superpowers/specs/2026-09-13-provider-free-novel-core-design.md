...
被拒绝、作废或冲突的尝试版本不产生可引用的永久 ID。后续任务只能引用已经进入权威状态的永久 ID。首次创建新权威对象时，ChatGPT 不得预填非空永久 ID 字段；这类输入必须 `REWRITE`，永久 ID 只能由 Core 在 `ACCEPTED` 时铸造。
...
离屏事件必须显式标记 `offscreen`，并引用允许它发生的章节约束或既有硬规则。Core 只检查引用，不假装理解这段离屏剧情写得是否合理。为兼容既有提交，`constraint_refs` 不强制去重；重复引用同一已存在约束不会增加额外语义。
...
每个新增知识都必须有明确来源。`fact_id` 是全局稳定事实标识：同一 `fact_id` 在不同角色知识中必须保持同一可读 statement 语义；不同事实或不同解释使用不同 `fact_id`。
...
关系 ID 的两端必须是两个不同的 canonical character；关系变化必须有事件证据；标签只是描述，不构成统一数轴。

### 18.5 伏笔、冲突和读者承诺

伏笔至少有稳定 ID、表层线索、真义、首次出现、强化记录、回收边界、依赖和后果；当前 Core 的可读 description 一旦已有非空值，普通状态推进不得改写，省略时继承前态。状态机应显式定义，例如：
