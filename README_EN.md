...
A chapter may introduce a new character, location, or tracked resource with `state_delta.character_add` / `state_delta.location_add` / `state_delta.resource_add` using an attempt-local `local_id`. Same-submission character/location/resource references may use those local IDs; only an ACCEPTED settlement allocates the permanent entity ID and returns it in `id_mappings`. Later tasks use the canonical ID. A newly created foreshadow also starts with an attempt-local `local_id`; ACCEPTED returns the permanent foreshadow ID in `id_mappings`, and later lifecycle transitions use only that canonical ID. A newly created reader promise follows the same rule: use an attempt-local `local_id`, then use the canonical `promise_id` returned by `id_mappings` for later transitions.
