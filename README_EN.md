...
Use `exchange/READY.json` as the only pointer to the current task. `exchange/STATUS.json` exposes the current authoritative `canon_root`, capability state, active task/attempt/block, plus `export_ready` and the complete deterministic `export_problems` list. Near the ending, use those problems to see obligations that may be omitted from the bounded task context; `export_ready=true` is only a prerequisite signal and final export still verifies Canon integrity. Drive multi-file sync is not atomic: proceed only when `STATUS.active_attempt_id == READY.attempt_id`, `STATUS.active_target == READY.target`, and `STATUS.block_id == READY.block_id` (both empty when unblocked); otherwise wait for sync and reread both files. ChatGPT then reads the matching outbox task/context and writes the submission to `exchange/inbox/<task-id>/<attempt-id>/`, with `manifest.json` written last.
...
A chapter may introduce a new character, location, or tracked resource with `state_delta.character_add` / `state_delta.location_add` / `state_delta.resource_add` using an attempt-local `local_id`. Same-submission character/location/resource references may use those local IDs; only an ACCEPTED settlement allocates the permanent entity ID and returns it in `id_mappings`. Later tasks use the canonical ID. A newly created foreshadow also starts with an attempt-local `local_id`; ACCEPTED returns the permanent foreshadow ID in `id_mappings`, and later lifecycle transitions use only that canonical ID. A story conflict is created with `state_delta.conflict.local_id`, canonical-character `participants`, escalation/close conditions, and event evidence; ACCEPTED returns a permanent `conflict-*` ID. Later transitions use `conflict_id` and follow `open → escalated → resolved`, with `open/escalated → retired` also allowed. Every conflict transition requires accepted event evidence, and final export requires all conflicts to be `resolved` or `retired`. A newly created reader promise follows the same rule: use an attempt-local `local_id`, then use the canonical `promise_id` returned by `id_mappings` for later transitions.
